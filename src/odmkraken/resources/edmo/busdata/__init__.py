...
class EDMOVehData(EDMOData):
    ...
    def get_timeframes_by_file(self, file_id: uuid.UUID) -> typing.List[VehicleTimeFrame]:
        """Return all vehicle timeframes contained in a given file."""
        with self.store.query(sql.vehicle_timeframes_for_file(schema=self.vehdata_schema), file_id) as cur:
            return [VehicleTimeFrame(*row) for row in cur]

    def get_timeframes_on(self, t0: pendulum.DateTime, tf: pendulum.DateTime) -> typing.List[VehicleTimeFrame]:
        """Return all vehicle timeframes contained in a given file."""
        with self.store.query(sql.vehicle_timeframes_for_period(schema=self.vehdata_schema), t0, tf) as cur:
            return [VehicleTimeFrame(*row) for row in cur]
    ...
    def extract_halts(self, tf: VehicleTimeFrame):
        ...
        with self.store.query(sql.extract_halts(schema=self.vehdata_schema), *tf.flat()) as cur:
            yield from cur
    ...
    def check_file_already_imported(self, checksum: bytes):
        with self.store.query(sql.file_by_checksum(schema=self.vehdata_schema), checksum) as cur:
            return cur.rowcount > 0

    def import_csv_file(self, handle, sep: str=',', table: str='raw_data'):
        self.store.run(sql.create_staging_table(staging_table=table, schema=self.vehdata_schema))
        self.store.copy_from(handle, (self.vehdata_schema, table), separator=sep)
        return self.store.fetchone(sql.len_staging_table(staging_table=table, schema=self.vehdata_schema))[0]
    ...
    def transform_data(self, file: pathlib.Path, checksum: bytes, table='raw_data'):
        with self.store.cursor() as cur:
            ...
            cur.execute(sql.extract_vehicles(staging_table=table, schema=self.vehdata_schema))
            new = ', '.join(f'{v[1]}-{v[2]} (id={v[0]})' for v in cur.fetchall())
            new = '(none)' if len(new) == 0 else new
            print(f'  new vehicles detected: {new}')

            print('extracting lines ...')
            cur.execute(sql.extract_lines(staging_table=table, schema=self.vehdata_schema))
            new = ', '.join(f'{v[1]} (id={v[0]})' for v in cur.fetchall())
            new = '(none)' if len(new) == 0 else new
            print(f'  new lines detected: {new}')

            cur.execute(sql.extract_stops(staging_table=table, schema=self.vehdata_schema))
            cur.execute(sql.extract_runs_with_timeframes(staging_table=table, schema=self.vehdata_schema))
            timeframes = cur.fetchall()

            cur.execute(sql.extract_pings(staging_table=table, schema=self.vehdata_schema))
            
            cur.execute(sql.add_data_file(schema=self.vehdata_schema), (str(file), checksum))
            file_id = cur.fetchone()[0]
        
            self.store.execute_batch(sql.add_file_timeframes(schema=self.vehdata_schema),
                                     [(str(file_id), *t) for t in timeframes], cursor=cur)
